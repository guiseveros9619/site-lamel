...
import { MousePointer2, Music, ShieldCheck, MapPin, Instagram, Youtube, Play, ArrowRight, Target, Heart, MessageCircle, Send, Flame, TrendingUp } from 'lucide-react'
...
const STEPS = [
  { id: 'audio', label: 'Conecte o Áudio' },
  { id: 'rules', label: 'Crie a Trend' },
  { id: 'geo', label: 'Regionalizado' },
  { id: 'ai', label: 'Validação InteligenteInteligente' },
  { id: 'bonus', label: 'Bônus e metas' },
  { id: 'pix', label: 'Pagamentos' },
]

export function StepByStepArtistas() {
  const container = useRef<HTMLDivElement>(null)
  const [activeStep, setActiveTab] = useState(0)

  useGSAP(
    () => {
      const cards = gsap.utils.toArray<HTMLElement>('.step-card-art')
      ...
      tlAudio
        ...
        // 3. Hover no TikTok e Clique
        .to('.audio-cursor', { x: 0, y: -30, duration: 0.5, ease: 'power1.inOut' }) // Move pro Tiktok
        ...
        // 4. Cursor move pro Instagram e Clique
        .to('.audio-btn-tk', { backgroundColor: '#1c1c1c', duration: 0.2 }) // Remove hover do Tiktok
        .to('.audio-cursor', { x: 15, y: 25, duration: 0.5, ease: 'power1.inOut' }, "<") // Move pro IG
        .to('.audio-btn-ig', { backgroundColor: '#2a2a2a', duration: 0.2 }, "<0.2") // Hover IG
      ...
      tlPix
        ...
        // --- ONDA 1 ---
        // A cascata desce e o budget reduz 275 reais de forma nativa e paralela (label 'onda1')
        ...
        // Prepara invisivelmente para a Onda 2 com novos valores
        .set('.pix-item', { y: -20, scale: 0.9 })
        .set('.pix-val-1', { text: "R$ 100,00" })
        .set('.pix-val-2', { text: "R$ 300,00" })
        .set('.pix-val-3', { text: "R$ 50,00" })

        // --- ONDA 2 ---
        // A segunda cascata desce e o budget reduz mais 450 reais (label 'onda2')
        .to('.pix-item', { y: 0, opacity: 1, scale: 1, duration: 0.5, stagger: 0.2, ease: 'back.out(1.5)' }, 'onda2')
        .to(budgetObj, { val: 14275, duration: 0.9, ease: 'power2.out', onUpdate: updateBudgetDisplay }, 'onda2')

        // Espera e recolhe a segunda onda
        .to('.pix-item', { y: 20, opacity: 0, duration: 0.3, stagger: 0.1, ease: 'power2.in', delay: 1 })

        // Finaliza ciclo (desliga o cofre e recarrega o saldo como se fosse um novo budget de cliente)
      ...
      <div className="container mx-auto max-w-6xl px-6 lg:px-8 mt-16 flex flex-col gap-24 lg:gap-0">
        ...
        <div className="step-card-art flex flex-col lg:flex-row items-center min-h-[70vh] border-b border-white/5 pb-24 lg:pb-0">
            <div className="flex-1 lg:pr-16 mb-12 lg:mb-0">
              ...
              <h2 className="step-text-anim text-4xl sm:text-5xl font-extrabold mb-6 tracking-tight">Conecte o Áudio</h2>
              <p className="step-text-anim text-xl text-zinc-400 font-medium leading-relaxed">
                 Cole o link do Áudio Oficial do seu lançamento no TikTok ou Reels. Ele será a chave mestra para milhares de criadores amplificarem o seu hit.
              </p>
           </div>
           ...
           <div className="flex-1 w-full lg:h-[420px] min-h-[300px] sm:min-h-[350px] lg:min-h-0 bg-[#1a1a1a] rounded-3xl p-8 lg:p-12 flex flex-col items-center justify-center relative overflow-hidden border border-white/5 shadow-2xl">
              ...
              <div className="audio-player relative w-full max-w-sm bg-[#111] rounded-3xl p-5 border border-white/5 shadow-2xl flex flex-col gap-5">
                 ...
                 {/* Top: Music Info (Like the reference image) */}
                 <div className="flex gap-4 items-center">
                   <div className="relative w-20 h-20 bg-red-600 rounded-xl overflow-hidden flex items-center justify-center shrink-0 shadow-lg">
                      <div className="absolute inset-0 opacity-20 bg-[url('/noise.png')] mix-blend-overlay"></div>
                      <div className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center border border-white/20 pl-1">
                        <Play size={16} className="text-white" fill="white" />
                      </div>
                   </div>
                   <div>
                      <h4 className="text-white font-bold text-lg leading-tight">Novo Hit</h4>
                      <p className="text-zinc-400 text-sm mb-1">Seu Artista</p>
                      <p className="text-zinc-600 text-xs">Toque para ouvir prévia</p>
                   </div>
                 </div>

                 {/* Bottom: Network Buttons */}
                 <div className="flex flex-col gap-2 border-t border-white/5 pt-5 relative">
                   <div className="audio-btn-tk w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-3 px-4 flex items-center gap-3">
                     <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-white"><path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5"/></svg>
                     <span className="text-sm font-bold text-white/90">Som no TikTok</span>
                   ...
                   <div className="audio-btn-ig w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-3 px-4 flex items-center gap-3">
                     ...
                     <span className="text-sm font-bold text-white/90">Áudio no Instagram</span>
                     <ArrowRight size={14} className="ml-auto text-zinc-600 -rotate-45" />
                   </div>

                   <div className="audio-btn-yt w-full bg-[#1c1c1c] border border-white/5 rounded-2xl py-3 px-4 flex items-center gap-3">
                     <Youtube size={18} className="text-white" />
                     <span className="text-sm font-bold text-white/90">Shorts no YouTube</span>
                     <ArrowRight size={14} className="ml-auto text-zinc-600 -rotate-45" />
                   </div>
        ...
        {/* Step 2: Regras da Campanha */}
        <div className="step-card-art flex flex-col lg:flex-row items-center min-h-[70vh] border-b border-white/5 pb-24 lg:pb-0 lg:flex-row-reverse">
            <div className="flex-1 lg:pl-16 mb-12 lg:mb-0">
              ...
              <h2 className="step-text-anim text-4xl sm:text-5xl font-extrabold mb-6 tracking-tight">Crie a Trend Perfeita</h2>
              <p className="step-text-anim text-xl text-zinc-400 font-medium leading-relaxed">
                 Quer uma dancinha? Um POV de lifestyle? Você direciona a criatividade. Defina as diretrizes, hashtags do lançamento e as metas de alcance para a música estourar do jeito certo.
              </p>
           </div>
              ...
              <div className="w-full max-w-sm bg-[#111] rounded-3xl p-6 border border-white/5 shadow-2xl flex flex-col gap-6 relative">
                 ...
                 <div>
                   ...
                   <div className="h-12 bg-[#1c1c1c] border border-white/5 rounded-xl px-4 flex items-center overflow-hidden">
                      <div className="rule-input-name h-full flex items-center whitespace-nowrap overflow-hidden">
                        <span className="text-white font-medium">Lançamento &ldquo;Noitada&rdquo;</span>
                      </div>
                   </div>
                 </div>
                 ...
                 <div>
                   ...
                   <div className="flex gap-2">
                      <div className="rule-tag-1 bg-brand-purple/20 border border-brand-purple/30 text-brand-purple px-3 py-1.5 rounded-lg text-sm font-bold">#Noitada</div>
                      <div className="rule-tag-2 bg-brand-purple/20 border border-brand-purple/30 text-brand-purple px-3 py-1.5 rounded-lg text-sm font-bold">#Hitlovers</div>
                   </div>
                 </div>
                 ...
                 <div>
                   ...
                   <div className="h-4 bg-[#1c1c1c] border border-white/5 rounded-full overflow-hidden p-0.5 relative">
                      <div className="rule-slider-fill h-full bg-gradient-to-r from-green-600 to-green-600 rounded-full w-0"></div>
                   </div>
                 </div>
        ...
        <div className="step-card-art flex flex-col lg:flex-row items-center min-h-[70vh] border-b border-white/5 pb-24 lg:pb-0">
            <div className="flex-1 lg:pr-16 mb-12 lg:mb-0">
              ...
              <h2 className="step-text-anim text-4xl sm:text-5xl font-extrabold mb-6 tracking-tight">Domínio Regionalizado</h2>
              <p className="step-text-anim text-lg sm:text-xl text-zinc-400 font-medium leading-relaxed">
                 Os maiores hits nacionais nascem na cultura local. Comece escolhendo uma região estratégica e nós pulverizamos a sua faixa em milhares de influenciadores na sua praça simultaneamente.
              </p>
           </div>
           ...
           <div className="flex-1 w-full lg:h-[420px] min-h-[300px] sm:min-h-[350px] lg:min-h-0 bg-[#1a1a1a] rounded-3xl relative overflow-hidden border border-white/5 shadow-2xl flex items-center justify-center">
              ...
              <div className="relative w-full h-full flex items-center justify-center">
                 ...
                 <div className="radar-label absolute bg-[#1c1c1c] text-white text-xs font-bold px-3 py-1 rounded-full border border-brand-purple/30 z-30 -translate-y-8">Goiânia, GO</div>
        ...
        <div className="step-card-art flex flex-col lg:flex-row items-center min-h-[70vh] border-b border-white/5 pb-24 lg:pb-0 lg:flex-row-reverse">
            <div className="flex-1 lg:pl-16 mb-12 lg:mb-0">
              ...
              <h2 className="step-text-anim text-4xl sm:text-5xl font-extrabold mb-6 tracking-tight">Validação Inteligente</h2>
              <p className="step-text-anim text-xl text-zinc-400 font-medium leading-relaxed">
                 Garantia total do seu investimento. A Inteligência Artificial escuta cada vídeo para auditar o áudio e as diretrizes e se as metas orgânicas foram batidas. Sem views falsas.
              </p>
           </div>
        ...
        <div className="step-card-art flex flex-col lg:flex-row items-center min-h-[70vh] border-b border-white/5 pb-24 lg:pb-0">
            <div className="flex-1 lg:pr-16 mb-12 lg:mb-0">
              ...
              <h2 className="step-text-anim text-4xl sm:text-5xl font-extrabold mb-6 tracking-tight">Bônus por desempenho</h2>
              <p className="step-text-anim text-xl text-zinc-400 font-medium leading-relaxed">
                 Incentive os criadores a irem além. O pagamento base é liberado na meta, mas se o vídeo explodir na timeline, destrava multiplicadores. Construa um exército focado em criar hits.
              </p>
           </div>
        ...
        {/* Step 6: Budget / Pix */}
        <div className="step-card-art flex flex-col lg:flex-row items-center min-h-[70vh] pb-24 lg:pb-0 lg:flex-row-reverse">
            <div className="flex-1 lg:pl-16 mb-12 lg:mb-0">
              ...
              <h2 className="step-text-anim text-4xl sm:text-5xl font-extrabold mb-6 tracking-tight">Escrow de Pagamentos</h2>
              <p className="step-text-anim text-xl text-zinc-400 font-medium leading-relaxed">
                 Esqueça planilhas e PIX manuais. O orçamento do lançamento fica blindado no contrato inteligente e os PIXs disparam sozinhos assim que a IA aprova o hype. Segurança em escala.
              </p>
           </div>
           ...
           <div className="flex-1 w-full lg:h-[420px] min-h-[300px] sm:min-h-[350px] lg:min-h-0 bg-[#1a1a1a] rounded-3xl p-8 lg:p-12 flex flex-col items-center justify-center relative overflow-hidden border border-white/5 shadow-2xl">
              ...
              <div className="w-full max-w-sm flex justify-center gap-4 relative z-10">
                 {/* Item 1 */}
                 <div className="pix-item bg-green-500/10 border border-green-500/30 text-green-400 px-4 py-3 rounded-xl flex flex-col items-center shadow-lg text-center break-keep whitespace-nowrap min-w-[100px]">
                    <span className="text-xs font-bold text-zinc-400">PIX Pago</span>
                    <span className="font-bold pix-val-1 block">R$ 50,00</span>
                 </div>
                 {/* Item 2 */}
                 <div className="pix-item bg-green-500/10 border border-green-500/30 text-green-400 px-4 py-3 rounded-xl flex flex-col items-center shadow-lg translate-y-4 text-center break-keep whitespace-nowrap min-w-[100px]">
                    <span className="text-xs font-bold text-zinc-400">PIX Pago</span>
